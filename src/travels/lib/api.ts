...
import type { VehicleType, VehicleAmenity, Vehicle } from '@travels/types'

// Same-origin by default: the Vite dev proxy (and any same-host deployment)
// forwards /api/v1 to the unified FastAPI service. Override with VITE_TRAVELS_API_URL.
const API_URL = import.meta.env.VITE_TRAVELS_API_URL || '/api/v1'
  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      if (typeof window !== 'undefined') {
        ...
        window.location.href = '/travels/login'
      }
    }
...
export const getVehicleById = async (id: string): Promise<Vehicle> => {
  ...
  console.log('API_URL:', import.meta.env.VITE_TRAVELS_API_URL || '/api/v1')
  try {
