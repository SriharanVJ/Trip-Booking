...
const DialogContent = React.forwardRef<HTMLDivElement, DialogContentProps>(
  ({ className, children, showCloseButton = true, onClose, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'relative z-50 w-full max-w-lg bg-background rounded-3xl shadow-premium-lg border border-border/50',
        'animate-scale-in p-6 max-h-[90vh] overflow-auto',
        className
      )}
      {...props}
    >
      {showCloseButton && (
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-4 top-4 rounded-xl hover:bg-muted/50"
          onClick={onClose}
        >
          <X className="h-5 w-5" />
        </Button>
