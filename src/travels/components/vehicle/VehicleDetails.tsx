...
import { Button } from '@travels/components/ui/button'
import { Badge } from '@travels/components/ui/badge'
import { Card, CardContent, CardHeader, CardTitle } from '@travels/components/ui/card'
import { Separator } from '@travels/components/ui/separator'
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@travels/components/ui/tabs'
import { ScrollArea, ScrollBar } from '@travels/components/ui/scroll-area'
import { Skeleton } from '@travels/components/ui/skeleton'
import { cn } from '@travels/lib/utils'
import type { Vehicle, VehicleAmenity } from '@travels/types'
...
// Only the amenities the original app illustrated are covered; unmapped
// slugs are skipped below instead of crashing the details page.
const amenityInfo: Partial<Record<
  VehicleAmenity,
      ...
      <div className="relative aspect-[16/9] rounded-2xl overflow-hidden bg-charcoal-dark border border-gold/10">
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-xs hover:bg-black/80 p-3 rounded-full shadow-gold border border-gold/20 transition-all hover:scale-110"
            ...
            <button
              onClick={nextImage}
              className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-xs hover:bg-black/80 p-3 rounded-full shadow-gold border border-gold/20 transition-all hover:scale-110"
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          <button className="bg-black/60 backdrop-blur-xs hover:bg-black/80 p-3 rounded-full shadow-gold border border-gold/20 transition-all hover:scale-110">
          ...
          <div className="flex gap-3 pb-2">
            {images.map((image, index) => (
              <button
                ...
                className={cn(
                  'relative shrink-0 w-28 h-20 rounded-xl overflow-hidden border-2 transition-all hover:scale-105',
                  index === currentIndex
    ...
    <Card className="glass-luxury-card border-gold/10">
      ...
      <CardContent className="pt-6">
        <div className="grid md:grid-cols-2 gap-4">
          {amenities.filter((amenity) => amenityInfo[amenity]).map((amenity) => (
            <div
              key={amenity}
            ...
            >
              <div className="shrink-0 p-3 rounded-xl bg-gold/10 border border-gold/20 group-hover:border-gold/30 transition-colors">
            ...
            <div key={review.id} className="border-b border-gold/10 pb-6 last:border-0 last:pb-0">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-full bg-linear-to-br from-gold/20 to-gold/10 flex items-center justify-center text-gold font-display font-bold border border-gold/30">
                    {review.avatar}
                  </div>
                  <div>
  ...
  return (
    <Card className="glass-luxury-card border-gold/20 sticky top-4 shadow-luxury-lg">
      ...
      <CardContent className="pt-6 space-y-6">
        ...
        <div className="space-y-3">
          <Button
            className="w-full h-14 bg-linear-to-r from-gold to-gold-light text-black hover:from-gold-light hover:to-gold font-display font-semibold text-lg rounded-2xl shadow-gold-lg shimmer-gold"
            onClick={onBookNow}
          >
            Book This Vehicle
