...
import { useSearchParams } from '@travels/lib/navigation'
import {
  MapPin,
  Users,
  Star,
...
import { Card, CardContent, CardFooter } from '@travels/components/ui/card'
...
export function VehicleCard({ vehicle, onViewDetails, variant = 'default', priority = false }: VehicleCardProps) {
  ...
  const buildBookingUrl = () => {
    ...
    return `/travels/vehicles/${vehicle.id}?${params.toString()}`
  }
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        <Link to="/travels/vehicles/$id" params={{ id: vehicle.id }}>
          <div className="flex">
            {/* Image */}
            <div className="relative w-48 h-32 overflow-hidden bg-black shrink-0">
              {!imageLoaded && !imageError && (
                <Skeleton className="absolute inset-0 h-full w-full bg-charcoal-dark" />
              )}
              {imageError ? (
                <div className="flex items-center justify-center h-full bg-linear-to-br from-charcoal to-black text-warm-white-dark/50">
              ...
              <Badge
                className={cn(
                  'absolute top-3 left-3 px-3 py-1 text-xs font-semibold border backdrop-blur-xs',
            ...
            <CardContent className="p-5 flex-1 flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between mt-3">
                ...
                <Button
                  size="sm"
                  className="h-9 px-4 bg-linear-to-r from-gold to-gold-light text-black hover:from-gold-light hover:to-gold font-semibold rounded-lg shadow-gold shimmer-gold"
                  onClick={(e) => {
                    e.preventDefault()
    ...
    >
      <Link to="/travels/vehicles/$id" params={{ id: vehicle.id }}>
        <div className="relative h-64 overflow-hidden bg-black">
          ...
          <div className="absolute top-5 left-5">
            <Badge
              className={cn(
                'px-4 py-2 border font-semibold shadow-gold backdrop-blur-xs',
                typeConfig.badgeClass
              )}
            >
          ...
          {/* Featured Badge */}
          {isFeatured && (
            <Badge className="absolute top-5 right-5 bg-linear-to-r from-gold to-gold-light text-black border-0 shadow-gold-lg font-semibold">
      ...
      <CardContent className="p-6 space-y-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <Link to="/travels/vehicles/$id" params={{ id: vehicle.id }} className="flex-1">
      ...
      <CardFooter className="p-6 pt-0 flex gap-3">
        <Button
          ...
          onClick={() => (window.location.href = `/travels/vehicles/${vehicle.id}`)}
        >
          View Details
